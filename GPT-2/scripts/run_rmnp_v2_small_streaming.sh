...
RUN_NAME="rmnp_v2-small-${DATASET_SHORT}-lr${LR}-rlr${RMNP_LR}-b1_${BETA1}-b2_${BETA2}-wd${WD}-rwd${RMNP_WD}-it${MAX_ITERS}"
LOG_FILE="${OUTPUT_DIR}/${RUN_NAME}_$(date +%Y%m%d_%H%M%S).log"

echo "rmnp_v2-small lr=${LR} rlr=${RMNP_LR} wd=${WD} iters=${MAX_ITERS} gpus=${GPUS}" | tee ${LOG_FILE}

torchrun --standalone --nproc_per_node=${GPUS} \
    MARS/train_rmnp_v2_streaming.py \
    config/train_gpt2_small_rmnp_v2_streaming.py \
