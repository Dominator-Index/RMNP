...
class RNNPS(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.02, momentum=0.95, nesterov=True, weight_decay=0.):
        ...
        # Timing for row normalization
        self.norm_time_step = 0.0
        self.norm_time_total = 0.0
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group['params']):
                ...
                if i % int(self.world_size) == int(self.rank):
                    ...
                    g = row_normalize(g)
                    torch.cuda.synchronize()
                    self.norm_time_step += time.perf_counter() - _start
                    g *= max(1, g.size(0)/g.size(1))**0.5   # TODO: Change Coefficient   g *= 0.2 * max(g.size(0), g.size(1))
                    # g *= g.size(1) ** 0.5
